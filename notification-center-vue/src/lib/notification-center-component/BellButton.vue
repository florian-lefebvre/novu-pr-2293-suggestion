...
const { unseenCount, bellButtonClass, gradientDotClass, colors } = props;

const color = ref(colors.unseenBadgeColor);
const isRegularColor = computed(() => typeof color.value === "string");

const gradientDot = computed(() => ({
  stopColor: isRegularColor.value
    ? (color.value as string)
    : (color.value as ISvgStopColor).stopColor,
  offsetStopColor: isRegularColor.value
    ? (color.value as string)
    : (color.value as ISvgStopColor).stopColorOffset,
}));
</script>
